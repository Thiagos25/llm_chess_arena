...
system_template = """
    ...
    1. Your move, using the following format: My move: "Move" (in SAN notation, in English).
    2. Criticize the human player's move, be aggressive and disrespectful in a maximum of 2 sentences, remember this is a psychological game! The mentally strongest player wins.
    3. The explanation, in Portuguese, of why you chose the move, in a maximum of 3 sentences.
    """
...
# Função para obter o movimento do LLM (GPT-4) com crítica
def get_move_llm(llm_chain, last_move, board, node, color, alert_msg=False):
    ...
    # Captura a crítica ao humano
    critic_match = re.search(r"Criticize the human player's move:\s*(.*)", move_raw)

    if critic_match:
        human_criticism = critic_match.group(1).strip()
        print(f"Crítica do GPT-4: {human_criticism}")
