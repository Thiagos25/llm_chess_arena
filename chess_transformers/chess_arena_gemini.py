...
# Definindo o prompt para a LLM da IA (peças pretas)
system_template = """
...
print("============")
print(f"New Game: Human (White) vs Gemini-Pro (Black)")
board = chess.Board()
game = chess.pgn.Game()
node = game
...
while not board.is_game_over():
    # Jogador Humano (Branco)
    move1 = None
    while move1 is None:
        print("\nSua vez! Informe o seu movimento no formato SAN (ex: e4, Nf3): ")
        move1 = input().strip()
        try:
            board.push_san(move1)
            node = node.add_variation(chess.Move.from_uci(board.parse_san(move1).uci()))
        except ValueError:
